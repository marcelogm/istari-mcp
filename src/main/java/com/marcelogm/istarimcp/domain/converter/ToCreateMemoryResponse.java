package com.marcelogm.istarimcp.domain.converter;

import com.marcelogm.istarimcp.api.memory.CreateMemoryResponse;
import com.marcelogm.istarimcp.api.memory.MemorySummaryResponse;
import com.marcelogm.istarimcp.domain.model.MemoryNode;
import jakarta.inject.Singleton;

import java.util.List;
import java.util.function.BiFunction;

@Singleton
public class ToCreateMemoryResponse implements BiFunction<MemoryNode, List<MemorySummaryResponse>, CreateMemoryResponse> {

    @Override
    public CreateMemoryResponse apply(MemoryNode memoryNode, List<MemorySummaryResponse> memorySimilarityRespons) {
        return new CreateMemoryResponse(
                memoryNode.id(),
                memoryNode.name(),
                memoryNode.description(),
                memorySimilarityRespons
        );
    }
}
